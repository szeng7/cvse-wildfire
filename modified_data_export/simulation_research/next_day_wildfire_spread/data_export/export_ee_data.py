...
def _get_all_feature_bands():
  ...
  return (ee_utils.DATA_BANDS[ee_utils.DataType.ELEVATION_SRTM] +
          ...
          ee_utils.DATA_BANDS[ee_utils.DataType.FORECAST]+ #ADDED for modified dataset
          ['PrevFireMask'])
...
def _get_all_image_collections():
  ...
  image_collections = {
      ...
      'forecast': #ADDED for modified dataset
          ee_utils.get_image_collection(ee_utils.DataType.FORECAST), #ADDED for modified dataset
  }
  time_sampling = {
      'drought':
      ...
      'forecast': #ADDED for modified dataset
          ee_utils.DATA_TIME_SAMPLING[ee_utils.DataType.FORECAST], #ADDED for modified dataset
  }
...
def _get_time_slices(
  ...
  forecast = image_collections['forecast'].filterDate(
      window_start.advance(-lag - time_sampling['forecast'], 'day'),
      window_start.advance(-lag, 'day')).filter('forecast_hours == 24').median().reproject(
          projection.atScale(resampling_scale)).resample('bicubic') # ADDED for modified dataset
  return [drought, vegetation, weather, prev_fire, fire, forecast, detection] #forecast ADDED for modified dataset
